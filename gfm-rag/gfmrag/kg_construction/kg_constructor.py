"""
================================================================================
FILE: kg_constructor.py - Xây dựng Knowledge Graph từ văn bản
================================================================================

MÔ TẢ TỔNG QUAN:
File này xây dựng Knowledge Graph (Đồ thị tri thức) từ documents văn bản thông qua:
1. Open Information Extraction (OpenIE) - Trích xuất entities và relations
2. Entity Linking - Chuẩn hóa và link entities  
3. Graph Augmentation - Thêm cạnh similarity (tùy chọn)
4. Finalization - Tạo KG hoàn chỉnh

QUY TRÌNH 4 BƯỚC:
┌─────────────────────────────────────────────────────────────────────┐
│ BƯỚC 1: OPEN IE → Trích xuất triples từ text                        │
│ Input: "Bill Gates founded Microsoft"                               │
│ Output: [["Bill Gates", "founded", "Microsoft"]]                    │
├─────────────────────────────────────────────────────────────────────┤
│ BƯỚC 2: CLEANING → Làm sạch và validate triples                    │
│ - Chuẩn hóa entity names                                           │
│ - Loại bỏ triples lỗi                                              │
├─────────────────────────────────────────────────────────────────────┤
│ BƯỚC 3: AUGMENTATION → Thêm cạnh similarity (nếu bật)              │
│ - Tìm entities tương tự                                            │
│ - Thêm cạnh "equivalent"                                           │
├─────────────────────────────────────────────────────────────────────┤
│ BƯỚC 4: FINALIZATION → Tạo kg.txt và document2entities.json        │
└─────────────────────────────────────────────────────────────────────┘

FILES:
Input:  dataset_corpus.json (thư mục raw/)
Output: kg.txt, document2entities.json (thư mục processed/stage1/)
Cache:  openie_results.jsonl, passage_info.json (thư mục tmp/)

USAGE:
    constructor = KGConstructor.from_config(config)
    triples = constructor.create_kg(data_root="./data", data_name="my_dataset")
    doc2ent = constructor.get_document2entities(data_root="./data", data_name="my_dataset")
================================================================================
"""
